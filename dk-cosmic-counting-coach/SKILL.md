...
> **Path resolution (cross-project use):** At the start of each session, resolve the
> skill directory once:
> ```bash
> SKILL_DIR=$(realpath ~/.claude/skills/dk-cosmic-counting-coach)
> ```
> All `manuals-indexed/` references below mean `$SKILL_DIR/manuals-indexed/`.
> All `manuals/` references mean `$SKILL_DIR/manuals/`.
...
The skill is non-functional until at least one PDF in `$SKILL_DIR/manuals/` has
been indexed into `$SKILL_DIR/manuals-indexed/`. Run from any directory:
...
1. Translate the user's phrasing into COSMIC vocabulary first — map natural
   language to canonical terms (e.g. "sends data" → Exit, "reads a record" →
   Read, "user submits a form" → Entry, "stores to database" → persistent
   storage). Then extract 3–8 keyword phrases from those translated terms.
...
The user supplies measurer JSON (see `dataMovements` schema in the reference doc) **and** the original source artifact (the
...
- No vector embeddings or semantic search — LLM vocabulary translation (step 1
  of Q&A oracle) bridges the natural-language gap; grep over heading-chunked
